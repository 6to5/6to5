...
code += `\ndeclare module "@babel/types" {
  ${indentLines(lines, 2)}
  ${generateDefaultExport(lines)}
}\n`;

function generateDefaultExport(moduleExports) {
  const functionExportPattern = /\bdeclare\s+export\s+function\s+(\w+)\b/;
  const renamedExportPattern = /\bdeclare\s+export\s*\{\s*(_\w+)\s+as\s+(\w+)\b/;

  const moduleExportEntries = {}; // map of exported name to Flow ref name
  moduleExports.forEach(line => {
    let match = line.match(functionExportPattern);
    if (match) {
      moduleExportEntries[match[1]] = match[1];
    } else {
      match = line.match(renamedExportPattern);
      if (match) {
        moduleExportEntries[match[2]] = match[1];
      }
    }
  });

  return `declare export default {
    ${indentLines(
      Object.keys(moduleExportEntries).map(
        publicName =>
          `${publicName}: typeof ${moduleExportEntries[publicName]},`
      ),
      4
    )}
  };`;
...
function indentLines(lines, padding) {
  const paddingStr = new Array(padding + 1).join(" ");
  return lines
    .join("\n")
    .replace(/\n/g, "\n" + paddingStr)
    .trim();
